/* PrismJS 1.29.0
https://prismjs.com/download.html#themes=prism-tomorrow&languages=markup+css+clike+javascript+bash+css-extras+markup-templating+php+python+ruby+typoscript&plugins=line-highlight+line-numbers+show-language+inline-color+previewers+unescaped-markup+toolbar+copy-to-clipboard */
	...
	Object.defineProperty(Prism.languages.markup.tag, 'addAttribute', {
		value: function (a, e) {
			Prism.languages.markup.tag.inside['special-attr'].push({
				...
				lookbehind: !0,
				inside: { 'attr-name': /^[^\s=]+/, 'attr-value': { pattern: /=[\s\S]+/, inside: { value: { pattern: /(^=\s*(["']|(?!["'])))\S[\s\S]*(?=\2$)/, lookbehind: !0, alias: [e, 'language-' + e], inside: Prism.languages[e] }, punctuation: [{ pattern: /^=/, alias: 'attr-equals' }, /"|'/] } } },
			});
		},
	}),
...
!(function (e) {
	var t =
		...
		n = {
			...
			variable: [
				{ pattern: /\$?\(\([\s\S]+?\)\)/, greedy: !0, inside: { variable: [{ pattern: /(^\$\(\([\s\S]+)\)\)/, lookbehind: !0 }, /^\$\(\(/], number: /\b0x[\dA-Fa-f]+\b|(?:\b\d+(?:\.\d*)?|\B\.\d+)(?:[Ee]-?\d+)?/, operator: /--|\+\+|\*\*=?|<<=?|>>=?|&&|\|\||[=!+\-*/%<>^&|]=?|[?~:]/, punctuation: /\(\(?|\)\)?|,|;/ } },
			...
			t = {
				gradient: {
								...
								? (e[s] = (function (e, s, t) {
										...
										return (
											/^(?:-?(?:\d+(?:\.\d+)?|\.\d+)(?:deg|rad)|to\b|top|right|bottom|left)/.test(t[0]) &&
												...
												(i.indexOf('top') >= 0
													? (i = i.indexOf('left') >= 0 ? 'to bottom right' : i.indexOf('right') >= 0 ? 'to bottom left' : 'to bottom')
													: i.indexOf('bottom') >= 0
													? (i = i.indexOf('left') >= 0 ? 'to top right' : i.indexOf('right') >= 0 ? 'to top left' : 'to top')
													: i.indexOf('left') >= 0
													? (i = 'to right')
													: i.indexOf('right') >= 0
													? (i = 'to left')
													: e && (i.indexOf('deg') >= 0 ? (i = 90 - parseFloat(i) + 'deg') : i.indexOf('rad') >= 0 && (i = Math.PI / 2 - parseFloat(i) + 'rad'))),
											s + '(' + i + ',' + t.join(',') + ')'
										);
								  })(i, a, n))
								: a.indexOf('radial') >= 0
					...
					languages: {
						...
						less: !0,
						sass: [
							{ lang: 'sass', before: 'punctuation', inside: 'inside', root: Prism.languages.sass && Prism.languages.sass['variable-line'] },
							{ lang: 'sass', before: 'punctuation', inside: 'inside', root: Prism.languages.sass && Prism.languages.sass['property-line'] },
						],
						scss: !0,
						stylus: [
							{ lang: 'stylus', before: 'func', inside: 'rest', root: Prism.languages.stylus && Prism.languages.stylus['property-declaration'].inside },
							{ lang: 'stylus', before: 'func', inside: 'rest', root: Prism.languages.stylus && Prism.languages.stylus['variable-declaration'].inside },
						],
					},
				},
				angle: {
					...
					languages: {
						css: !0,
						less: !0,
						markup: { lang: 'markup', before: 'punctuation', inside: 'inside', root: Prism.languages.markup && Prism.languages.markup.tag.inside['attr-value'] },
						sass: [
							{ lang: 'sass', inside: 'inside', root: Prism.languages.sass && Prism.languages.sass['property-line'] },
							{ lang: 'sass', before: 'operator', inside: 'inside', root: Prism.languages.sass && Prism.languages.sass['variable-line'] },
						],
						scss: !0,
						stylus: [
							{ lang: 'stylus', before: 'func', inside: 'rest', root: Prism.languages.stylus && Prism.languages.stylus['property-declaration'].inside },
				...
				color: {
					...
					languages: {
						...
						scss: !1,
						stylus: [
							{ lang: 'stylus', before: 'hexcode', inside: 'rest', root: Prism.languages.stylus && Prism.languages.stylus['property-declaration'].inside },
							{ lang: 'stylus', before: 'hexcode', inside: 'rest', root: Prism.languages.stylus && Prism.languages.stylus['variable-declaration'].inside },
						],
					},
				},
				easing: {
					create: function () {
						new Prism.plugins.Previewer(
							'easing',
							function (e) {
								var s = (e = { linear: '0,0,1,1', ease: '.25,.1,.25,1', 'ease-in': '.42,0,1,1', 'ease-out': '0,0,.58,1', 'ease-in-out': '.42,0,.58,1' }[e] || e).match(/-?(?:\d+(?:\.\d+)?|\.\d+)/g);
								if (4 === s.length) {
					...
					tokens: { easing: { pattern: /\bcubic-bezier\((?:-?(?:\d+(?:\.\d+)?|\.\d+),\s*){3}-?(?:\d+(?:\.\d+)?|\.\d+)\)\B|\b(?:ease(?:-in)?(?:-out)?|linear)(?=\s|[;}]|$)/i, inside: { function: /[\w-]+(?=\()/, punctuation: /[(),]/ } } },
					languages: {
						css: !0,
						less: !0,
						sass: [
							{ lang: 'sass', inside: 'inside', before: 'punctuation', root: Prism.languages.sass && Prism.languages.sass['variable-line'] },
							{ lang: 'sass', inside: 'inside', root: Prism.languages.sass && Prism.languages.sass['property-line'] },
						],
						scss: !0,
						stylus: [
				...
				if (e.language && i[e.language] && !i[e.language].initialized) {
					...
					Array.isArray(a) || (a = [a]),
						a.forEach(function (a) {
							var n, r, o, l;
							!0 === a ? ((n = 'important'), (r = e.language), (a = e.language)) : ((n = a.before || 'important'), (r = a.inside || a.lang), (o = a.root || Prism.languages), (l = a.skip), (a = e.language)),
								!l && Prism.languages[a] && (Prism.languages.insertBefore(r, n, t[s].tokens, o), (e.grammar = Prism.languages[a]), (i[e.language] = { initialized: !0 }));
						});
				}
			}
		}),
